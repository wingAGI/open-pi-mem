...
def _encode_image_part(image_path: Path) -> dict[str, Any]:
    mime_type = "image/png"
...
def _extract_text_and_thoughts_from_response(payload: dict[str, Any]) -> tuple[str, str, int | None]:
    texts: list[str] = []
    thoughts: list[str] = []
    ...
    return "\n".join(texts).strip(), "\n".join(thoughts).strip(), thoughts_token_count
...
def _run_single_window(
    *,
    ...
    url = f"{base_url.rstrip('/')}/models/{model_name}:generateContent"
    body = {
        "contents": [
            {
                "role": "user",
                "parts": [{"text": prompt}] + [_encode_image_part(path) for path in image_paths],
            }
        ],
        "generationConfig": {
    ...
    for attempt in range(1, max(max_retries, 1) + 1):
        try:
            ...
            text, thought_summary, thoughts_token_count = _extract_text_and_thoughts_from_response(payload)
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Run RMBench high-level episode inference with the Gemini API using a multi-frame window."
    )
    ...
    parser.add_argument("--base-url", default="https://generativelanguage.googleapis.com/v1beta")
    ...
    if args.report_dir:
        ...
        with tempfile.TemporaryDirectory(prefix="rmbench_hl_gemini_multiframe_") as tmp_dir:
